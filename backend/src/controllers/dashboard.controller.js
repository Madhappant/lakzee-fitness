...
const getDashboardStats = async (req, res) => {
    try {
        // 1. Active Members
        const activeMembersCount = await app_1.prisma.user.count({
            where: { role: 'MEMBER' }
        });
        // 2. Today's Check-ins
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const checkInsCount = await app_1.prisma.attendance.count({
            where: {
                date: {
                    gte: today,
                }
        ...
        const newSignupsCount = await app_1.prisma.user.count({
            where: {
                role: 'MEMBER',
                createdAt: {
                    gte: firstDayOfMonth
        ...
        const recentSubscriptions = await app_1.prisma.subscription.findMany({
            where: {
                createdAt: {
                    gte: firstDayOfMonth
                },
        ...
        const recentCheckIns = await app_1.prisma.attendance.findMany({
            take: 5,
        ...
        const expiringIn7Days = await app_1.prisma.subscription.count({
            where: {
                status: 'ACTIVE',
        ...
        // Expired
        const expiredMembers = await app_1.prisma.subscription.count({
            where: {
                OR: [
                    { status: 'EXPIRED' },
                    { endDate: { lt: today } }
        ...
        const allMembersWithDob = await app_1.prisma.memberProfile.findMany({
            where: { dob: { not: null } },
            select: { dob: true }
        });
        ...
        const last14DaysSubs = await app_1.prisma.subscription.findMany({
        ...
        const recentSubsList = await app_1.prisma.subscription.findMany({
            take: 5,
