...
router.get('/', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (userRole === 'ADMIN' || userRole === 'RECEPTIONIST') {
            // ADMIN NOTIFICATIONS
            // 1. Pending Payments
            const pendingInvoices = await app_1.prisma.invoice.findMany({
            ...
            const expiringSubscriptions = await app_1.prisma.subscription.findMany({
                where: {
            ...
            if (memberProfile) {
                ...
                const myPendingInvoices = await app_1.prisma.invoice.findMany({
                ...
                const myExpiringSubscriptions = await app_1.prisma.subscription.findMany({
                    where: {
